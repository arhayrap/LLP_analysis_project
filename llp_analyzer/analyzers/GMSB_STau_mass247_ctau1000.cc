#include "GMSB_STau_mass247_ctau1000.h"
...
void GMSB_STau_mass247_ctau1000::Analyze(bool isData, int options, string outputfilename, string analysisTag) {
    ...
    if (outfilename == "") outfilename = "/eos/user/a/arhayrap/OUTPUT_DATA/GMSB_STau_mass247_ctau1000.root";
    TFile * outFile;
    ...
    TNtuple * Gen_part      = new TNtuple("Gen particles", "Gen particles", "pt:eta:phi:e:mass:x:y:z");
    TNtuple * Tau           = new TNtuple("Tau", "Tau", "pt:eta:phi:e:mass:x:y:z");
    TNtuple * Gravitino     = new TNtuple("Gravitino", "Gravitino", "pt:eta:phi:e:mass");
    TNtuple * Gen_csc_dt    = new TNtuple("Gen CSC DT", "Gen CSC DT", "pt:eta:phi:e:mass"); // matched stau
    TNtuple * Gen_csc_dt_manual_l       = new TNtuple("Gen CSC DT manual l", "Gen CSC DT manual l", "pt:eta:phi:e:dr_csc:dr_dt:x:y:z:grav_pt:grav_e:tau_pt:tau_e:time"); // manualy matched stau
    TNtuple * Gen_csc_dt_manual_h       = new TNtuple("Gen CSC DT manual h", "Gen CSC DT manual h", "pt:eta:phi:e:dr_csc:dr_dt:x:y:z:grav_pt:grav_e:tau_pt:tau_e:time"); // manualy matched stau
    TNtuple * Gen_csc_dt_manual_notrack = new TNtuple("Gen CSC DT manual no track", "Gen CSC DT manual no track", "pt:eta:phi:e:dr_csc:dr_dt:x:y:z:grav_pt:grav_e:tau_pt:tau_e:time"); // manualy matched stau
    TNtuple * Gen_csc_dt_manual_nomatch = new TNtuple("Gen CSC DT manual nomatch", "Gen CSC DT manual nomatch",   "pt:eta:phi:e:dr_csc:dr_dt:x:y:z:grav_pt:grav_e:tau_pt:tau_e"); // manualy not matched stau
    TNtuple * Gen_csc_dt_rechits_400    = new TNtuple("Gen CSC DT rechits no track 400", "Gen CSC DT rechits no track 400", "N:pt"); // number of rechits 400
    TNtuple * Gen_csc_dt_rechits_500    = new TNtuple("Gen CSC DT rechits no track 500", "Gen CSC DT rechits no track 500", "N:pt"); // number of rechits 500
    TNtuple * Gen_csc_dt_rechits_600    = new TNtuple("Gen CSC DT rechits no track 600", "Gen CSC DT rechits no track 600", "N:pt"); // number of rechits 600
    TNtuple * Gen_csc_dt_rechits_800    = new TNtuple("Gen CSC DT rechits no track 800", "Gen CSC DT rechits no track 800", "N:pt"); // number of rechits 800

    TNtuple * Gen_csc_dt_rechits_400_l    = new TNtuple("Gen CSC DT rechits no track 400 lepton", "Gen CSC DT rechits no track 400 lepton", "N:pt"); // number of rechits 400
    TNtuple * Gen_csc_dt_rechits_500_l    = new TNtuple("Gen CSC DT rechits no track 500 lepton", "Gen CSC DT rechits no track 500 lepton", "N:pt"); // number of rechits 500
    TNtuple * Gen_csc_dt_rechits_600_l    = new TNtuple("Gen CSC DT rechits no track 600 lepton", "Gen CSC DT rechits no track 600 lepton", "N:pt"); // number of rechits 600
    TNtuple * Gen_csc_dt_rechits_800_l    = new TNtuple("Gen CSC DT rechits no track 800 lepton", "Gen CSC DT rechits no track 800 lepton", "N:pt"); // number of rechits 800

    TNtuple * Gen_csc_dt_rechits_400_h    = new TNtuple("Gen CSC DT rechits no track 400 hadron", "Gen CSC DT rechits no track 400 hadron", "N:pt"); // number of rechits 400
    TNtuple * Gen_csc_dt_rechits_500_h    = new TNtuple("Gen CSC DT rechits no track 500 hadron", "Gen CSC DT rechits no track 500 hadron", "N:pt"); // number of rechits 500
    TNtuple * Gen_csc_dt_rechits_600_h    = new TNtuple("Gen CSC DT rechits no track 600 hadron", "Gen CSC DT rechits no track 600 hadron", "N:pt"); // number of rechits 600
    TNtuple * Gen_csc_dt_rechits_800_h    = new TNtuple("Gen CSC DT rechits no track 800 hadron", "Gen CSC DT rechits no track 800 hadron", "N:pt"); // number of rechits 800
    ...
    for (Long64_t jentry = 0; jentry < fChain -> GetEntries(); jentry++) {
        ...
        if (!isData) {
            ...
            if (!isData) //twin higgs model && !hnl_model
            {
                ...
                double STau_1_E = 0, STau_2_E = 0;
                double STau_1_PT = 0, STau_2_PT = 0;
                double STau_1_ETA = 0, STau_2_ETA = 0;
                double STau_1_PHI = 0, STau_2_PHI = 0;
                double STau_1_decay_X = 100000, STau_2_decay_X = 100000;
                double STau_1_decay_Y = 100000, STau_2_decay_Y = 100000;
                double STau_1_decay_Z = 100000, STau_2_decay_Z = 100000;
                bool STau_1_Check = true, STau_2_Check = true;
                int STau_1_index = -100, STau_2_index = -100;

                double Tau_1_E = 0, Tau_2_E = 0;
                double Tau_1_PT = 0, Tau_2_PT = 0;
                double Tau_1_ETA = 0, Tau_2_ETA = 0;
                double Tau_1_PHI = 0, Tau_2_PHI = 0;
                double Tau_1_decay_X = 100000, Tau_2_decay_X = 100000;
                double Tau_1_decay_Y = 100000, Tau_2_decay_Y = 100000;
                double Tau_1_decay_Z = 100000, Tau_2_decay_Z = 100000;
                bool Tau_1_Check = true, Tau_2_Check = true;
                int Tau_1_index = -100, Tau_2_index = -100;

                double Grav_1_E = 0, Grav_2_E = 0;
                double Grav_1_PT = 0, Grav_2_PT = 0;
                double Grav_1_ETA = 0, Grav_2_ETA = 0;
                double Grav_1_PHI = 0, Grav_2_PHI = 0;
                bool Grav_1_Check = true, Grav_2_Check = true;

                double Nmu_1_E = 0, Nmu_2_E = 0;
                double Nmu_1_PT = 0, Nmu_2_PT = 0;
                double Nmu_1_ETA = 0, Nmu_2_ETA = 0;
                double Nmu_1_PHI = 0, Nmu_2_PHI = 0;
                bool Nmu_1_Check = true, Nmu_2_Check = true;

                double Ne_1_E = 0, Ne_2_E = 0;
                double Ne_1_PT = 0, Ne_2_PT = 0;
                double Ne_1_ETA = 0, Ne_2_ETA = 0;
                double Ne_1_PHI = 0, Ne_2_PHI = 0;
                bool Ne_1_Check = true, Ne_2_Check = true;

                double Ntau_1_E = 0, Ntau_2_E = 0;
                double Ntau_1_PT = 0, Ntau_2_PT = 0;
                double Ntau_1_ETA = 0, Ntau_2_ETA = 0;
                double Ntau_1_PHI = 0, Ntau_2_PHI = 0;
                bool Ntau_1_Check = true, Ntau_2_Check = true;

                double E_1_E = 0, E_2_E = 0;
                double E_1_PT = 0, E_2_PT = 0;
                double E_1_ETA = 0, E_2_ETA = 0;
                double E_1_PHI = 0, E_2_PHI = 0;
                bool E_1_Check = true, E_2_Check = true;

                double Mu_1_E = 0, Mu_2_E = 0;
                double Mu_1_PT = 0, Mu_2_PT = 0;
                double Mu_1_ETA = 0, Mu_2_ETA = 0;
                double Mu_1_PHI = 0, Mu_2_PHI = 0;
                bool Mu_1_Check = true, Mu_2_Check = true;
                ...
                for (int i = 0; i < nGenParticle; i++) {
                    ...
                    if (STau_1_Check && gParticleId[i] == 1000015 && gParticleStatus[i] != 1) { // stau
                    ...
                    if (STau_2_Check && gParticleId[i] == -1000015 && gParticleStatus[i] != 1) { // -stau
                    ...
                    if (Tau_1_Check && gParticleId[i] == 15 && gParticleStatus[i] == 2 && gParticleMotherId[i] == 1000015) { // tau-
                        STau_1_decay_X = gParticleProdVertexX[i];
                        STau_1_decay_Y = gParticleProdVertexY[i];
                        STau_1_decay_Z = gParticleProdVertexZ[i];
                        STau_1_index = gParticleMotherIndex[i];
                        Tau_1_E = gParticleE[i];
                        Tau_1_PT = gParticlePt[i];
                        Tau_1_ETA = gParticleEta[i];
                        Tau_1_PHI = gParticlePhi[i];
                        Tau_1_Check = false;
                        Tau_1_index = i;
                    }
                    if (Tau_2_Check && gParticleId[i] == -15 && gParticleStatus[i] == 2 && gParticleMotherId[i] == -1000015) { // tau
                        STau_2_decay_X = gParticleProdVertexX[i];
                        STau_2_decay_Y = gParticleProdVertexY[i];
                        STau_2_decay_Z = gParticleProdVertexZ[i];
                        STau_2_index = gParticleMotherIndex[i];
                        Tau_2_E = gParticleE[i];
                        Tau_2_PT = gParticlePt[i];
                        Tau_2_ETA = gParticleEta[i];
                        Tau_2_PHI = gParticlePhi[i];
                        Tau_2_Check = false;
                        Tau_2_index = i;
                    }
                    if (Grav_1_Check && gParticleId[i] == 1000039 && gParticleStatus[i] == 1 && gParticleMotherId[i] == 1000015) { // Gravitino
                        Grav_1_E = gParticleE[i];
                        Grav_1_PT = gParticlePt[i];
                        Grav_1_ETA = gParticleEta[i];
                        Grav_1_PHI = gParticlePhi[i];
                        Grav_1_Check = false;
                    }
                    if (Grav_2_Check && gParticleId[i] == 1000039 && gParticleStatus[i] == 1 && gParticleMotherId[i] == -1000015) { // Gravitino
                        Grav_2_E = gParticleE[i];
                        Grav_2_PT = gParticlePt[i];
                        Grav_2_ETA = gParticleEta[i];
                        Grav_2_PHI = gParticlePhi[i];
                        Grav_2_Check = false;
                    }
                    // leptons, neutrinos and pi0 mesons
                    if (Ne_1_Check && gParticleId[i] == 12 && gParticleMotherId[i] == 15) { // neutrino e-
                        Ne_1_E = gParticleE[i];
                        Ne_1_PT = gParticlePt[i];
                        Ne_1_ETA = gParticleEta[i];
                        Ne_1_PHI = gParticlePhi[i];
                        Ne_1_Check = false;
                    }
                    if (Nmu_1_Check && gParticleId[i] == 14 && gParticleMotherId[i] == 15) { // neutrino mu-
                        Nmu_1_E = gParticleE[i];
                        Nmu_1_PT = gParticlePt[i];
                        Nmu_1_ETA = gParticleEta[i];
                        Nmu_1_PHI = gParticlePhi[i];
                        Nmu_1_Check = false;
                    }
                    if (Ntau_1_Check && gParticleId[i] == 16 && gParticleMotherId[i] == 15) { // neutrino tau-
                        Tau_1_decay_X = gParticleProdVertexX[i];
                        Tau_1_decay_Y = gParticleProdVertexY[i];
                        Tau_1_decay_Z = gParticleProdVertexZ[i];
                        Ntau_1_E = gParticleE[i];
                        Ntau_1_PT = gParticlePt[i];
                        Ntau_1_ETA = gParticleEta[i];
                        Ntau_1_PHI = gParticlePhi[i];
                        Ntau_1_Check = false;
                    }

                    if (Ne_2_Check && gParticleId[i] == -12 && gParticleMotherId[i] == -15) { // neutrino e+
                        Ne_2_E = gParticleE[i];
                        Ne_2_PT = gParticlePt[i];
                        Ne_2_ETA = gParticleEta[i];
                        Ne_2_PHI = gParticlePhi[i];
                        Ne_2_Check = false;
                    }
                    if (Nmu_2_Check && gParticleId[i] == -14 && gParticleMotherId[i] == -15) { // neutrino mu+
                        Nmu_2_E = gParticleE[i];
                        Nmu_2_PT = gParticlePt[i];
                        Nmu_2_ETA = gParticleEta[i];
                        Nmu_2_PHI = gParticlePhi[i];
                        Nmu_2_Check = false;
                    }
                    if (Ntau_2_Check && gParticleId[i] == -16 && gParticleMotherId[i] == -15) { // neutrino tau+
                        Tau_2_decay_X = gParticleProdVertexX[i];
                        Tau_2_decay_Y = gParticleProdVertexY[i];
                        Tau_2_decay_Z = gParticleProdVertexZ[i];
                        Ntau_2_E = gParticleE[i];
                        Ntau_2_PT = gParticlePt[i];
                        Ntau_2_ETA = gParticleEta[i];
                        Ntau_2_PHI = gParticlePhi[i];
                        Ntau_2_Check = false;
                    }

                    if (Mu_1_Check && gParticleId[i] == 13 && gParticleMotherId[i] == 15) { // mu-
                        Mu_1_E = gParticleE[i];
                        Mu_1_PT = gParticlePt[i];
                        Mu_1_ETA = gParticleEta[i];
                        Mu_1_PHI = gParticlePhi[i];
                        Mu_1_Check = false;
                    }
                    if (E_1_Check && gParticleId[i] == 11 && gParticleMotherId[i] == 15) { // e-
                        E_1_E = gParticleE[i];
                        E_1_PT = gParticlePt[i];
                        E_1_ETA = gParticleEta[i];
                        E_1_PHI = gParticlePhi[i];
                        E_1_Check = false;
                    }

                    if (Mu_2_Check && gParticleId[i] == -13 && gParticleMotherId[i] == -15) { // mu+
                        Mu_2_E = gParticleE[i];
                        Mu_2_PT = gParticlePt[i];
                        Mu_2_ETA = gParticleEta[i];
                        Mu_2_PHI = gParticlePhi[i];
                        Mu_2_Check = false;
                    }
                    if (E_2_Check && gParticleId[i] == -11 && gParticleMotherId[i] == -15) { // e+
                        E_2_E = gParticleE[i];
                        E_2_PT = gParticlePt[i];
                        E_2_ETA = gParticleEta[i];
                        E_2_PHI = gParticlePhi[i];
                        E_2_Check = false;
                    }
                }
                ...
                vector<double> ids1;
                vector<double> ids2;
                int id1 = 77777777, id2 = 77777777;
                //  && gParticleMotherIndex[i] == Tau_1_index /*(gParticleMotherIndex[i]) == Tau_1_index*/
                for (int i = 0; i < nGenParticle; i++) {
                    if ((gParticleMotherId[i]) ==  15) {
                        //lepton1 = true;
                        //check_1 = false;
                        //id1 = gParticleId[i];
                        ids1.push_back(gParticleId[i]);
                    }
                    if ((gParticleMotherId[i]) == -15) {
                        //lepton2 = true;
                        //check_2 = false;
                        //id2 = gParticleId[i];
                        ids2.push_back(gParticleId[i]);
                    }
                }
                
                for (auto i = 0; i < ids1.size(); i++) {
            	    cout<<"||||||| "<<ids1[i]<<endl;
            	    if (ids1[i] == 11 || ids1[i] == -12 || ids1[i] == 13 || ids1[i] == -14) {
            		lepton1 = true;
            		break;
            	    }
                }
                ...
                for (auto i = 0; i < ids2.size(); i++) {
            	    cout<<"||||||| "<<ids2[i]<<endl;
            	    if (ids2[i] == -11 || ids2[i] == 12 || ids2[i] == -13 || ids2[i] == 14) {
            		lepton2 = true;
            		break;
            	    }
                }
                ...
                /*
                for (int i = 0; i < nGenParticle; i++) {
                    if (check_1 && (gParticleId[i] == 11 || gParticleId[i] == -12 || gParticleId[i] == 13 || gParticleId[i] == -14) && gParticleId[i] != 16 && gParticlePt[i] > 5.0  && (gParticleMotherId[i]) == 15) {
                        //lepton1 = true;
                        //check_1 = false;
                        //id1 = gParticleId[i];
                        ids1.push_back(gParticleId[i]);
                    }
                    else if (check_1 && (gParticleId[i] != 11 && gParticleId[i] != -12 && gParticleId[i] != 13 && gParticleId[i] != -14) && gParticleId[i] != 16 && gParticlePt[i] > 5.0  && (gParticleMotherId[i]) == 15) {
                        //hadron1 = true;
                        //check_1 = false;
                        //id1 = gParticleId[i];
                        ids1.push_back(gParticleId[i]);
                    ...
                    if (check_2 && (gParticleId[i] == -11 || gParticleId[i] == 12 || gParticleId[i] == -13 || gParticleId[i] == 14) && gParticleId[i] != -16 && gParticlePt[i] > 5.0 && (gParticleMotherId[i]) == -15) {
                        //lepton2 = true;
                        //check_2 = false;
                        //id2 = gParticleId[i];
                        ids2.push_back(gParticleId[i]);
                    }
                    else if (check_2 && (gParticleId[i] != -11 && gParticleId[i] != 12 && gParticleId[i] != -13 && gParticleId[i] != 14) && gParticleId[i] != -16 && gParticlePt[i] > 5.0 && (gParticleMotherId[i]) == -15) {
                        //hadron2 = true;
                        //check_2 = false;
                        //id2 = gParticleId[i];
                        ids2.push_back(gParticleId[i]);
                ...
                */
                ...
                cout << id1 <<" <(============)> "<< id2 << endl;
                cout << "/*-///*/jdsdsf " << hadron1 << " " << hadron2 << "                    " << lepton1 << " " << lepton2 << endl;
                ...
                if (normal_event) {
                    ...
                    if (STau1_InMuonSystem) {
                        /*
                        if (lepton1) {
        	                  lepton_and_hadron -> Fill(1);
                        }
                        if (hadron1) {
        	                  lepton_and_hadron -> Fill(0);
                        }
                        */
                        double tau_px = STau_1_PT * cos(STau_1_PHI);
                        double tau_py = STau_1_PT * sin(STau_1_PHI);
                        double tau_pz = STau_1_PT * sinh(STau_1_ETA);
                        double tau_P = sqrt(pow(tau_px, 2) + pow(tau_py, 2) + pow(tau_pz, 2));
                        double tau_mass = sqrt(pow(Tau_1_E, 2) - pow(tau_P, 2));
                        double grav_px = Grav_1_PT * cos(Grav_1_PHI);
                        double grav_py = Grav_1_PT * sin(Grav_1_PHI);
                        double grav_pz = Grav_1_PT * sinh(Grav_1_ETA);
                        double grav_P = sqrt(pow(grav_px, 2) + pow(grav_py, 2) + pow(grav_pz, 2));
                        double grav_mass = sqrt(pow(Grav_1_E, 2) - pow(grav_P, 2));

                        Tau            -> Fill(Tau_1_PT, Tau_1_ETA, Tau_1_PHI, Tau_1_E, tau_mass, Tau_1_decay_X, Tau_1_decay_Y, Tau_1_decay_Z);
                        Gravitino      -> Fill(Grav_1_PT, Grav_1_ETA, Grav_1_PHI, Grav_1_E, grav_mass);
                        Neutrino_tau   -> Fill(Ntau_1_PT, Ntau_1_ETA, Ntau_1_PHI, Ntau_1_E);
                        Neutrino_other -> Fill(Ne_1_PT, Ne_1_ETA, Ne_1_PHI, Ne_1_E);
                        Neutrino_other -> Fill(Nmu_1_PT, Nmu_1_ETA, Nmu_1_PHI, Nmu_1_E);
                        Leptons_other  -> Fill(Mu_1_PT, Mu_1_ETA, Mu_1_PHI, Mu_1_E);
                        Leptons_other  -> Fill(E_1_PT, E_1_ETA, E_1_PHI, E_1_E);

                        if (Ntau_1_PT == 0.0) {
                            cout << "&&&--- " << Ntau_1_PT << "     " << Ntau_1_ETA << "     " << Ntau_1_PHI << "     " << Ntau_1_E << "     " << endl;
                        }
                    }

                    if (STau2_InMuonSystem) {
                        /*
                        if (lepton2 && STau2_InMuonSystem) {
        	                  lepton_and_hadron -> Fill(1);
                        }
                        if (hadron2 && STau2_InMuonSystem) {
        	                  lepton_and_hadron -> Fill(0);
                        }
                        */

                        double tau_px = STau_2_PT * cos(STau_2_PHI);
                        double tau_py = STau_2_PT * sin(STau_2_PHI);
                        double tau_pz = STau_2_PT * sinh(STau_2_ETA);
                        double tau_P = sqrt(pow(tau_px, 2) + pow(tau_py, 2) + pow(tau_pz, 2));
                        double tau_mass = sqrt(pow(Tau_2_E, 2) - pow(tau_P, 2));
                        double grav_px = Grav_2_PT * cos(Grav_2_PHI);
                        double grav_py = Grav_2_PT * sin(Grav_2_PHI);
                        double grav_pz = Grav_2_PT * sinh(Grav_2_ETA);
                        double grav_P = sqrt(pow(grav_px, 2) + pow(grav_py, 2) + pow(grav_pz, 2));
                        double grav_mass = sqrt(pow(Grav_2_E, 2) - pow(grav_P, 2));

                        Tau            -> Fill(Tau_2_PT, Tau_2_ETA, Tau_2_PHI, Tau_2_E, tau_mass, Tau_2_decay_X, Tau_2_decay_Y, Tau_2_decay_Z);
                        Gravitino      -> Fill(Grav_2_PT, Grav_2_ETA, Grav_2_PHI, Grav_2_E, grav_mass);
                        Neutrino_tau   -> Fill(Ntau_2_PT, Ntau_2_ETA, Ntau_2_PHI, Ntau_2_E);
                        Neutrino_other -> Fill(Ne_2_PT, Ne_2_ETA, Ne_2_PHI, Ne_2_E);
                        Neutrino_other -> Fill(Nmu_2_PT, Nmu_2_ETA, Nmu_2_PHI, Nmu_2_E);
                        Leptons_other  -> Fill(Mu_2_PT, Mu_2_ETA, Mu_2_PHI, Mu_2_E);
                        Leptons_other  -> Fill(E_2_PT, E_2_ETA, E_2_PHI, E_2_E);

                        if (Ntau_2_PT == 0.0) {
                            cout << "&&&--- " << Ntau_2_PT << "     " << Ntau_2_ETA << "     " << Ntau_2_PHI << "     " << Ntau_2_E << "     " << endl;
                        }
                    }
                    ...
                    if (STau1_InMuonSystem || STau2_InMuonSystem) {
                        ...
                        tau_px = STau_2_PT * cos(STau_2_PHI);
                        tau_py = STau_2_PT * sin(STau_2_PHI);
                        tau_pz = STau_2_PT * sinh(STau_2_ETA);
                        tau_P = sqrt(pow(tau_px, 2) + pow(tau_py, 2) + pow(tau_pz, 2));
                        tau_mass = sqrt(pow(Tau_2_E, 2) - pow(tau_P, 2));
                        Tau -> Fill(Tau_2_PT, Tau_2_ETA, Tau_2_PHI, Tau_2_E, tau_mass, Tau_2_decay_X, Tau_2_decay_Y, Tau_2_decay_Z);
                        ...
                        grav_px = Grav_2_PT * cos(Grav_2_PHI);
                        grav_py = Grav_2_PT * sin(Grav_2_PHI);
                        grav_pz = Grav_2_PT * sinh(Grav_2_ETA);
                        grav_P = sqrt(pow(grav_px, 2) + pow(grav_py, 2) + pow(grav_pz, 2));
                        grav_mass = sqrt(pow(Grav_2_E, 2) - pow(grav_P, 2));
                        Gravitino -> Fill(Grav_2_PT, Grav_2_ETA, Grav_2_PHI, Grav_2_E, grav_mass);
                        ...
                        Pid_0 -> Fill(Pi0d_1_PT, Pi0d_1_ETA, Pi0d_1_PHI, Pi0d_1_E, Pi0d_1_decay_X, Pi0d_1_decay_Y, Pi0d_1_decay_Z);
                        Pi_0  -> Fill(Pi0_1_PT, Pi0_1_ETA, Pi0_1_PHI, Pi0_1_E);
                        
                        Pid_0 -> Fill(Pi0d_2_PT, Pi0d_2_ETA, Pi0d_2_PHI, Pi0d_2_E, Pi0d_2_decay_X, Pi0d_2_decay_Y, Pi0d_2_decay_Z);
                        Pi_0  -> Fill(Pi0_2_PT, Pi0_2_ETA, Pi0_2_PHI, Pi0_2_E);
                        ...
                        for (int i = 0; i < nCscRechitClusters; i++) {
                            ...
                            if (dist_cluster_csc < 0.5) {
                                ...
                                All_csc_dt_manual -> Fill(gParticleId[matched_csc], gParticlePt[matched_csc], gParticleEta[matched_csc], gParticlePhi[matched_csc], gParticleE[matched_csc], dist_cluster_csc);
                            }
                        }
                        ...
                        for (int i = 0; i < nDtRechitClusters; i++) {
                            ...
                            if (dist_cluster_dt < 0.5) {
                                ...
                                All_csc_dt_manual -> Fill(gParticleId[matched_dt], gParticlePt[matched_dt], gParticleEta[matched_dt], gParticlePhi[matched_dt], gParticleE[matched_dt], dist_cluster_dt);
                            }
                        }*/

                        cout << endl << endl;
                        
                        bool isolated_track_1 = true;
                        double pt_sum_1 = 0;
                        bool isolated_track_2 = true;
                        double pt_sum_2 = 0;
                        // Closeset Track to STau_1
                        int Index_Matched_To_STau_1 = -100;
                        double dist_To_STau_1 = 10;
                        ...
                        if (STau1_InMuonSystem && STau_1_PT > 0.0 && dist_To_STau_1 < 0.5) {
                    	    for (int ii = 0; ii < nTracks; ii++) {
                    		double phi_dist = fabs(track_Phi[Index_Matched_To_STau_1] - track_Phi[ii]);
                    		double eta_dist = fabs(track_Eta[Index_Matched_To_STau_1] - track_Eta[ii]);
                    		if (phi_dist > 3.1415) phi_dist = 6.283 - phi_dist;
                    		double dr = sqrt(pow(phi_dist, 2) + pow(eta_dist, 2));
                    		if (dr < 0.4 && ii != Index_Matched_To_STau_1) {
                    		    pt_sum_1 += track_Pt[ii];
                    		}
                    	    }
                    	    if (pt_sum_1 > track_Pt[Index_Matched_To_STau_1]/10) {
                    		isolated_track_1 = false;
                    	    }
                        }
                        
                        if (STau2_InMuonSystem && STau_2_PT > 0.0 && dist_To_STau_2 < 0.5) {
                    	    for (int ii = 0; ii < nTracks; ii++) {
                    		double phi_dist = fabs(track_Phi[Index_Matched_To_STau_2] - track_Phi[ii]);
                    		double eta_dist = fabs(track_Eta[Index_Matched_To_STau_2] - track_Eta[ii]);
                    		if (phi_dist > 3.1415) phi_dist = 6.283 - phi_dist;
                    		double dr = sqrt(pow(phi_dist, 2) + pow(eta_dist, 2));
                    		if (dr < 0.4 && ii != Index_Matched_To_STau_2) {
                    		    pt_sum_2 += track_Pt[ii];
                    		}
                    	    }
                    	    if (pt_sum_2 > track_Pt[Index_Matched_To_STau_2]/10) {
                    		isolated_track_2 = false;
                    	    }
                        }
                        
                        int n_stau_normal = 0;
                        cout << endl;
                        if (STau1_InMuonSystem && STau_1_PT > 0.0) {
                            n_stau_normal++;
                            ...
                            Gen_part -> Fill(STau_1_PT, STau_1_ETA, STau_1_PHI, STau_1_E, mass, STau_1_decay_X, STau_1_decay_Y, STau_1_decay_Z);
                            if (lepton1) {
                                lepton_and_hadron -> Fill(1);
                            }
                            if (hadron1) {
                        ...
                        if (STau2_InMuonSystem && STau_2_PT > 0.0) {
                            n_stau_normal++;
                            ...
                            Gen_part -> Fill(STau_2_PT, STau_2_ETA, STau_2_PHI, STau_2_E, mass, STau_2_decay_X, STau_2_decay_Y, STau_2_decay_Z);
                            if (lepton2) {
                                lepton_and_hadron -> Fill(1);
                            }
                            if (hadron2) {
                        ...
                        if (STau1_InMuonSystem && Index_Matched_To_STau_1 >= 0 && isolated_track_1) {
                        ...
                        if (STau2_InMuonSystem && Index_Matched_To_STau_2 >= 0 && isolated_track_2) {
                        ...
                        double dist_cluster_csc_1 = 10;
                        double dist_cluster_dt_1 = 10;
                        int matched_csc_2 = -100;
                        int matched_dt_2 = -100;
                        double dist_cluster_csc_2 = 10;
                        double dist_cluster_dt_2 = 10;
                        ...
                        if (STau1_InMuonSystem && STau_1_PT > 0.0 && Index_Matched_To_STau_1 >= 0) {
                            ...
                            for (int i = 0; i < nCscRechitClusters; i++) {
                                ...
                                float pz_matched = gParticlePt[cscRechitCluster_match_gParticle_index[i]] * sinh(gParticleEta[cscRechitCluster_match_gParticle_index[i]]);
                                float mass_matched = sqrt(pow(gParticleE[cscRechitCluster_match_gParticle_index[i]], 2) - pow(px_matched, 2) - pow(py_matched, 2) - pow(pz_matched, 2));
                                cout << gParticlePt[cscRechitCluster_match_gParticle_index[i]] << "   " << gParticleEta[cscRechitCluster_match_gParticle_index[i]] << "   " << gParticlePhi[cscRechitCluster_match_gParticle_index[i]] << "   " << gParticleE[cscRechitCluster_match_gParticle_index[i]] << "   " << mass_matched << endl;
                                Gen_csc_dt -> Fill(gParticlePt[cscRechitCluster_match_gParticle_index[i]], gParticleEta[cscRechitCluster_match_gParticle_index[i]], gParticlePhi[cscRechitCluster_match_gParticle_index[i]], gParticleE[cscRechitCluster_match_gParticle_index[i]], mass_matched);
                            }
                            ...
                            for (int i = 0; i < nDtRechitClusters; i++) {
                                ...
                                float pz_matched = gParticlePt[dtRechitCluster_match_gParticle_index[i]] * sinh(gParticleEta[dtRechitCluster_match_gParticle_index[i]]);
                                float mass_matched = sqrt(pow(gParticleE[dtRechitCluster_match_gParticle_index[i]], 2) - pow(px_matched, 2) - pow(py_matched, 2) - pow(pz_matched, 2));
                                cout << gParticlePt[dtRechitCluster_match_gParticle_index[i]] << "   " << gParticleEta[dtRechitCluster_match_gParticle_index[i]] << "   " << gParticlePhi[dtRechitCluster_match_gParticle_index[i]] << "   " << gParticleE[dtRechitCluster_match_gParticle_index[i]] << "   " << mass_matched << endl;
                                Gen_csc_dt -> Fill(gParticlePt[dtRechitCluster_match_gParticle_index[i]], gParticleEta[dtRechitCluster_match_gParticle_index[i]], gParticlePhi[dtRechitCluster_match_gParticle_index[i]], gParticleE[dtRechitCluster_match_gParticle_index[i]], mass_matched);
                            }
                        ...
                        if (STau2_InMuonSystem && STau_2_PT > 0.0 /*&& Index_Matched_To_STau_2 >= 0*/ ) {
                            matched_csc_2 = -100;
                            matched_dt_2 = -100;
                            dist_cluster_csc_2 = 10;
                            dist_cluster_dt_2 = 10;
                        ...
                        if (hadron1) {
                            if (dist_cluster_dt_1 < 0.5) {
                                Gen_csc_dt_manual_h -> Fill(STau_1_PT, STau_1_ETA, STau_1_PHI, STau_1_E, dist_cluster_dt_1, dist_cluster_csc_1, STau_1_decay_X, STau_1_decay_Y, STau_1_decay_Z, Grav_1_PT, Grav_1_E, Tau_1_PT, Tau_1_E, dtRechitClusterTime[matched_dt_1]);
                            }
                            if (dist_cluster_csc_1 < 0.5) {
                                Gen_csc_dt_manual_h -> Fill(STau_1_PT, STau_1_ETA, STau_1_PHI, STau_1_E, dist_cluster_dt_1, dist_cluster_csc_1, STau_1_decay_X, STau_1_decay_Y, STau_1_decay_Z, Grav_1_PT, Grav_1_E, Tau_1_PT, Tau_1_E, cscRechitClusterTime[matched_csc_1]);
                            }
                        }

                        if (lepton1) {
                            if (dist_cluster_dt_1 < 0.5) {
                                Gen_csc_dt_manual_l -> Fill(STau_1_PT, STau_1_ETA, STau_1_PHI, STau_1_E, dist_cluster_dt_1, dist_cluster_csc_1, STau_1_decay_X, STau_1_decay_Y, STau_1_decay_Z, Grav_1_PT, Grav_1_E, Tau_1_PT, Tau_1_E, dtRechitClusterTime[matched_dt_1]);
                            }
                            if (dist_cluster_csc_1 < 0.5) {
                                Gen_csc_dt_manual_l -> Fill(STau_1_PT, STau_1_ETA, STau_1_PHI, STau_1_E, dist_cluster_dt_1, dist_cluster_csc_1, STau_1_decay_X, STau_1_decay_Y, STau_1_decay_Z, Grav_1_PT, Grav_1_E, Tau_1_PT, Tau_1_E, cscRechitClusterTime[matched_csc_1]);
                            }
                        }

                        if (STau1_InMuonSystem && (dist_cluster_dt_1 >= 0.5 && dist_cluster_csc_1 >= 0.5)) {
                            Gen_csc_dt_manual_nomatch -> Fill(STau_1_PT, STau_1_ETA, STau_1_PHI, STau_1_E, dist_cluster_dt_1, dist_cluster_csc_1, STau_1_decay_X, STau_1_decay_Y, STau_1_decay_Z, Grav_1_PT, Grav_1_E, Tau_1_PT, Tau_1_E);
                        }

                        if (hadron2) {
                            if (dist_cluster_dt_2 < 0.5) {
                                Gen_csc_dt_manual_h -> Fill(STau_2_PT, STau_2_ETA, STau_2_PHI, STau_2_E, dist_cluster_dt_2, dist_cluster_csc_2, STau_2_decay_X, STau_2_decay_Y, STau_2_decay_Z, Grav_2_PT, Grav_2_E, Tau_2_PT, Tau_2_E, dtRechitClusterTime[matched_dt_2]);
                            }
                            if (dist_cluster_csc_2 < 0.5) {
                                Gen_csc_dt_manual_h -> Fill(STau_2_PT, STau_2_ETA, STau_2_PHI, STau_2_E, dist_cluster_dt_2, dist_cluster_csc_2, STau_2_decay_X, STau_2_decay_Y, STau_2_decay_Z, Grav_2_PT, Grav_2_E, Tau_2_PT, Tau_2_E, cscRechitClusterTime[matched_csc_2]);
                            }
                        }

                        if (lepton2) {
                            if (dist_cluster_dt_2 < 0.5) {
                                Gen_csc_dt_manual_l -> Fill(STau_2_PT, STau_2_ETA, STau_2_PHI, STau_2_E, dist_cluster_dt_2, dist_cluster_csc_2, STau_2_decay_X, STau_2_decay_Y, STau_2_decay_Z, Grav_2_PT, Grav_2_E, Tau_2_PT, Tau_2_E, dtRechitClusterTime[matched_dt_2]);
                            }
                            if (dist_cluster_csc_2 < 0.5) {
                                Gen_csc_dt_manual_l -> Fill(STau_2_PT, STau_2_ETA, STau_2_PHI, STau_2_E, dist_cluster_dt_2, dist_cluster_csc_2, STau_2_decay_X, STau_2_decay_Y, STau_2_decay_Z, Grav_2_PT, Grav_2_E, Tau_2_PT, Tau_2_E, cscRechitClusterTime[matched_csc_2]);
                            }
                        }

                        if (STau2_InMuonSystem && (dist_cluster_dt_2 >= 0.5 && dist_cluster_csc_2 >= 0.5)) {
                            Gen_csc_dt_manual_nomatch -> Fill(STau_2_PT, STau_2_ETA, STau_2_PHI, STau_2_E, dist_cluster_dt_2, dist_cluster_csc_2, STau_2_decay_X, STau_2_decay_Y, STau_2_decay_Z, Grav_2_PT, Grav_2_E, Tau_2_PT, Tau_2_E);
                        }
                        ...
                        matched_csc_1 = -100;
                        matched_dt_1 = -100;
                        dist_cluster_csc_1 = 10;
                        dist_cluster_dt_1 = 10;
                        matched_csc_2 = -100;
                        matched_dt_2 = -100;
                        dist_cluster_csc_2 = 10;
                        dist_cluster_dt_2 = 10;
                        ...
                        if (dist_cluster_dt_1 < 0.5) {
                            Gen_csc_dt_manual_notrack -> Fill(STau_1_PT, STau_1_ETA, STau_1_PHI, STau_1_E, dist_cluster_dt_1, dist_cluster_csc_1, STau_1_decay_X, STau_1_decay_Y, STau_1_decay_Z, Grav_1_PT, Grav_1_E, Tau_1_PT, Tau_1_E, dtRechitClusterTime[matched_dt_1]);
                        }
                        if (dist_cluster_csc_1 < 0.5) {
                            Gen_csc_dt_manual_notrack -> Fill(STau_1_PT, STau_1_ETA, STau_1_PHI, STau_1_E, dist_cluster_dt_1, dist_cluster_csc_1, STau_1_decay_X, STau_1_decay_Y, STau_1_decay_Z, Grav_1_PT, Grav_1_E, Tau_1_PT, Tau_1_E, cscRechitClusterTime[matched_csc_1]);
                        }

                        if (dist_cluster_dt_2 < 0.5) {
                            Gen_csc_dt_manual_notrack -> Fill(STau_2_PT, STau_2_ETA, STau_2_PHI, STau_2_E, dist_cluster_dt_2, dist_cluster_csc_2, STau_2_decay_X, STau_2_decay_Y, STau_2_decay_Z, Grav_2_PT, Grav_2_E, Tau_2_PT, Tau_2_E, dtRechitClusterTime[matched_dt_2]);
                        }
                        if (dist_cluster_csc_2 < 0.5) {
                            Gen_csc_dt_manual_notrack -> Fill(STau_2_PT, STau_2_ETA, STau_2_PHI, STau_2_E, dist_cluster_dt_2, dist_cluster_csc_2, STau_2_decay_X, STau_2_decay_Y, STau_2_decay_Z, Grav_2_PT, Grav_2_E, Tau_2_PT, Tau_2_E, dtRechitClusterTime[matched_csc_2]);
                        }

                        int counter_1 = 0;
                        int counter_2 = 0;

                        if (STau_1_PT > 400 && STau_1_PT < 500 && Tau_1_PT > 100 && STau1_InMuonSystem) {
                            if (dist_cluster_csc_1 < 0.5 || dist_cluster_dt_1 < 0.5) {
                                for (int i = 0; i < ncscRechits; i++) {
                                    float deltaPhi = fabs(cscRechitsPhi[i] - STau_1_PHI);
                                    float deltaEta = cscRechitsEta[i] - STau_1_ETA;
                                    if (deltaPhi > 3.1415) deltaPhi = 6.283 - deltaPhi;
                                    float deltaR = sqrt(pow(deltaPhi, 2) + pow(deltaEta, 2));
                                    if (deltaR < 0.5) {
                                        counter_1++;
                                ...
                                for (int i = 0; i < nDtRechits; i++) {
                                    float deltaPhi = fabs(dtRechitPhi[i] - STau_1_PHI);
                                    float deltaEta = dtRechitEta[i] - STau_1_ETA;
                                    if (deltaPhi > 3.1415) deltaPhi = 6.283 - deltaPhi;
                                    float deltaR = sqrt(pow(deltaPhi, 2) + pow(deltaEta, 2));
                                    if (deltaR < 0.5) {
                                        counter_1++;
                                ...
                                Gen_csc_dt_rechits_400 -> Fill(counter_1, STau_1_PT);
                                if (lepton1){
                                    Gen_csc_dt_rechits_400_l -> Fill(counter_1, STau_1_PT);
				} else if (hadron1){
				    Gen_csc_dt_rechits_400_h -> Fill(counter_1, STau_1_PT);
				}
                            }
                        }
                        
                        if (STau_2_PT > 400 && STau_2_PT < 500 && Tau_2_PT > 100 && STau2_InMuonSystem) {
                            if (dist_cluster_csc_2 < 0.5 || dist_cluster_dt_2 < 0.5) {
                                for (int i = 0; i < ncscRechits; i++) {
                                    float deltaPhi = fabs(cscRechitsPhi[i] - STau_2_PHI);
                                    float deltaEta = cscRechitsEta[i] - STau_2_ETA;
                                    if (deltaPhi > 3.1415) deltaPhi = 6.283 - deltaPhi;
                                    float deltaR = sqrt(pow(deltaPhi, 2) + pow(deltaEta, 2));
                                    if (deltaR < 0.5) {
                                        counter_2++;
                                ...
                                for (int i = 0; i < nDtRechits; i++) {
                                    float deltaPhi = fabs(dtRechitPhi[i] - STau_2_PHI);
                                    float deltaEta = dtRechitEta[i] - STau_2_ETA;
                                    if (deltaPhi > 3.1415) deltaPhi = 6.283 - deltaPhi;
                                    float deltaR = sqrt(pow(deltaPhi, 2) + pow(deltaEta, 2));
                                    if (deltaR < 0.5) {
                                        counter_2++;
                                ...
                                Gen_csc_dt_rechits_400 -> Fill(counter_2, STau_2_PT);
                                if (lepton2){
                                    Gen_csc_dt_rechits_400_l -> Fill(counter_2, STau_2_PT);
				} else if (hadron2){
				    Gen_csc_dt_rechits_400_h -> Fill(counter_2, STau_2_PT);
				}
                            }
                        }
                        ...
                        if (STau_1_PT > 500 && STau_1_PT < 600 && Tau_1_PT > 100 && STau1_InMuonSystem) {
                            if (dist_cluster_csc_1 < 0.5 || dist_cluster_dt_1 < 0.5) {
                                for (int i = 0; i < ncscRechits; i++) {
                                    float deltaPhi = fabs(cscRechitsPhi[i] - STau_1_PHI);
                                    float deltaEta = cscRechitsEta[i] - STau_1_ETA;
                                ...
                                Gen_csc_dt_rechits_500 -> Fill(counter_1, STau_1_PT);
                                if (lepton1){
                                    Gen_csc_dt_rechits_500_l -> Fill(counter_1, STau_1_PT);
				} else if (hadron1){
				    Gen_csc_dt_rechits_500_h -> Fill(counter_1, STau_1_PT);
				}
                            }
                        }

                        if (STau_2_PT > 500 && STau_2_PT < 600 && Tau_2_PT > 100 && STau2_InMuonSystem) {
                            if (dist_cluster_csc_2 < 0.5 || dist_cluster_dt_2 < 0.5) {
                                for (int i = 0; i < ncscRechits; i++) {
                                    float deltaPhi = fabs(cscRechitsPhi[i] - STau_2_PHI);
                                    float deltaEta = cscRechitsEta[i] - STau_2_ETA;
                                ...
                                Gen_csc_dt_rechits_500 -> Fill(counter_2, STau_2_PT);
                                if (lepton2){
                                    Gen_csc_dt_rechits_500_l -> Fill(counter_2, STau_2_PT);
				} else if (hadron2){
				    Gen_csc_dt_rechits_500_h -> Fill(counter_2, STau_2_PT);
				}
                            }
                        }
                        ...
                        if (STau_1_PT > 600 && STau_1_PT < 800 && Tau_1_PT > 100 && STau1_InMuonSystem) {
                            if (dist_cluster_csc_1 < 0.5 || dist_cluster_dt_1 < 0.5) {
                                for (int i = 0; i < ncscRechits; i++) {
                                    float deltaPhi = fabs(cscRechitsPhi[i] - STau_1_PHI);
                                    float deltaEta = cscRechitsEta[i] - STau_1_ETA;
                                ...
                                Gen_csc_dt_rechits_600 -> Fill(counter_1, STau_1_PT);
                                if (lepton1){
                                    Gen_csc_dt_rechits_600_l -> Fill(counter_1, STau_1_PT);
				} else if (hadron1){
				    Gen_csc_dt_rechits_600_h -> Fill(counter_1, STau_1_PT);
				}
                            }
                        }

                        if (STau_2_PT > 600 && STau_2_PT < 800 && Tau_2_PT > 100 && STau2_InMuonSystem) {
                            if (dist_cluster_csc_2 < 0.5 || dist_cluster_dt_2 < 0.5) {
                                for (int i = 0; i < ncscRechits; i++) {
                                    float deltaPhi = fabs(cscRechitsPhi[i] - STau_2_PHI);
                                    float deltaEta = cscRechitsEta[i] - STau_2_ETA;
                                ...
                                Gen_csc_dt_rechits_600 -> Fill(counter_2, STau_2_PT);
                                if (lepton2){
                                    Gen_csc_dt_rechits_600_l -> Fill(counter_2, STau_2_PT);
				} else if (hadron2){
				    Gen_csc_dt_rechits_600_h -> Fill(counter_2, STau_2_PT);
				}
                            }
                        }
                        ...
                        if (STau_1_PT > 800 && Tau_1_PT > 100 && STau1_InMuonSystem) {
                            if (dist_cluster_csc_1 < 0.5 || dist_cluster_dt_1 < 0.5) {
                                for (int i = 0; i < ncscRechits; i++) {
                                    float deltaPhi = fabs(cscRechitsPhi[i] - STau_1_PHI);
                                    float deltaEta = cscRechitsEta[i] - STau_1_ETA;
                                ...
                                Gen_csc_dt_rechits_800 -> Fill(counter_1, STau_1_PT);
                                if (lepton1){
                                    Gen_csc_dt_rechits_800_l -> Fill(counter_1, STau_1_PT);
				} else if (hadron1){
				    Gen_csc_dt_rechits_800_h -> Fill(counter_1, STau_1_PT);
				}
                            }
                        }

                        if (STau_2_PT > 800 && Tau_2_PT > 100 && STau2_InMuonSystem) {
                            if (dist_cluster_csc_2 < 0.5 || dist_cluster_dt_2 < 0.5) {
                                for (int i = 0; i < ncscRechits; i++) {
                                    float deltaPhi = fabs(cscRechitsPhi[i] - STau_2_PHI);
                                    float deltaEta = cscRechitsEta[i] - STau_2_ETA;
                                ...
                                Gen_csc_dt_rechits_800 -> Fill(counter_2, STau_2_PT);
                                if (lepton2){
                                    Gen_csc_dt_rechits_800_l -> Fill(counter_2, STau_2_PT);
				} else if (hadron2){
				    Gen_csc_dt_rechits_800_h -> Fill(counter_2, STau_2_PT);
				}
                            }
                        }
                        ...
                        /*
                                     if(STau1_InMuonSystem)
                                      {
                                        cout<<endl<<endl;
                                        printf ("%10.3f %10.3f %10.3f %10.3f %10.3f %10.3f %15d \n", STau_2_PT, STau_1_ETA, STau_1_PHI, STau_1_decay_X, STau_1_decay_Y, STau_1_decay_Z, STau1_InMuonSystem);
    ...
    if (!isData && signalScan) {
        ...
        Tau -> Write();
        Gen_csc_dt -> Write();
        Gen_csc_dt_manual_h -> Write();
        Gen_csc_dt_manual_l -> Write();
        Gen_csc_dt_rechits_400 -> Write();
        Gen_csc_dt_rechits_500 -> Write();
        Gen_csc_dt_rechits_600 -> Write();
        Gen_csc_dt_rechits_800 -> Write();
        
        Gen_csc_dt_rechits_400_l -> Write();
        Gen_csc_dt_rechits_500_l -> Write();
        Gen_csc_dt_rechits_600_l -> Write();
        Gen_csc_dt_rechits_800_l -> Write();
        
        Gen_csc_dt_rechits_400_h -> Write();
        Gen_csc_dt_rechits_500_h -> Write();
        Gen_csc_dt_rechits_600_h -> Write();
        Gen_csc_dt_rechits_800_h -> Write();
        Gen_csc_dt_manual_notrack -> Write();
