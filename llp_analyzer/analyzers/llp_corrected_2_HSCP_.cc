#include "llp_corrected_2_HSCP_.h"
...
void llp_corrected_2_HSCP_::Analyze(bool isData, int options, string outputfilename, string analysisTag) {
  ...
  TNtuple * Gen_part      = new TNtuple("Gen particles" ,"Gen particles" ,"pt:eta:phi");
  ...
  for (Long64_t jentry = 0; jentry < fChain -> GetEntries(); jentry++) {
	...
	//*********************************************************************
        //cout<<"Number of tracksssss: "<<nTracks<<" <-> "<<jentry<<" <-> "<<i<<endl;
        //*********************************************************************
...
// ******************************************************************************** My Changes
        ...
        double STau_1_E=0; double STau_1_PT=0; double STau_1_ETA=0; double STau_1_PHI=0, STau_1_MotherId=0;
        double STau_1_decay_X=0; double STau_1_decay_Y=0; double STau_1_decay_Z=0; bool STau_1_Check=true;
        double STau_2_E=0; double STau_2_PT=0; double STau_2_ETA=0; double STau_2_PHI=0, STau_2_MotherId=0;
        double STau_2_decay_X=0; double STau_2_decay_Y=0; double STau_2_decay_Z=0; bool STau_2_Check=true;

	for (int i = 0; i < nGenParticle; i++) 
	 {
	  if(STau_1_Check && gParticleId[i] == 1000015 && gParticleStatus[i] == 1)
	    {STau_1_E=gParticleE[i]; STau_1_PT=gParticlePt[i]; STau_1_ETA=gParticleEta[i]; STau_1_PHI=gParticlePhi[i]; STau_1_Check=false; STau_1_MotherId=gParticleMotherId[i]; continue;}
	  if(STau_2_Check && gParticleId[i] == -1000015 && gParticleStatus[i] == 1)
	    {STau_2_E=gParticleE[i]; STau_2_PT=gParticlePt[i]; STau_2_ETA=gParticleEta[i]; STau_2_PHI=gParticlePhi[i]; STau_2_Check=false; STau_2_MotherId=gParticleMotherId[i]; continue;}
	  if(gParticleId[i] == 15 && gParticleStatus[i] ==2 && gParticleMotherId[i] == 1000015)
            {STau_1_decay_X=gParticleProdVertexX[i]; STau_1_decay_Y=gParticleProdVertexY[i]; STau_1_decay_Z=gParticleProdVertexZ[i];}
	  if(gParticleId[i] == -15 && gParticleStatus[i] ==2 && gParticleMotherId[i] == -1000015)
            {STau_2_decay_X=gParticleProdVertexX[i]; STau_2_decay_Y=gParticleProdVertexY[i]; STau_2_decay_Z=gParticleProdVertexZ[i];}
	 }

//        cout<<endl<<endl<<STau_1_PT<<"   "<<STau_1_ETA<<"   "<<STau_1_PHI<<"   "<<STau_1_decay_X<<"   "<<STau_1_decay_Y<<"   "<<STau_1_decay_Z<<endl;
//        cout<<STau_2_PT<<"   "<<STau_2_ETA<<"   "<<STau_2_PHI<<"   "<<STau_2_decay_X<<"   "<<STau_2_decay_Y<<"   "<<STau_2_decay_Z<<endl<<endl<<endl;
...
*/
        ...
        bool eta_STau1 = (fabs(STau_1_ETA) < 2.4 && STau_1_PT > 0.0);
        bool eta_STau2 = (fabs(STau_2_ETA) < 2.4 && STau_2_PT > 0.0);
        ...
        if (eta_STau2)
         ...
         if (STau1_InMuonSystem || STau2_InMuonSystem)
         {
             ...
             int Track_Index_Matched_To_STau_1 = -100;
             double Track_dist_To_STau_1 = 0.4;
             ...
             for(int ii=0; ii<nTracks; ii++)
               {
                ...
                if(sqrt(pow(STau_1_ETA - track_Eta[ii], 2) + pow(phi_dist, 2)) < Track_dist_To_STau_1 )
                 {
                  Track_dist_To_STau_1 = sqrt(pow(STau_1_ETA - track_Eta[ii], 2) + pow(phi_dist, 2));
                  Track_Index_Matched_To_STau_1=ii;
                 }
               }
             // Closeset Track to STau_2
             int Track_Index_Matched_To_STau_2 = -100;
             double Track_dist_To_STau_2 = 0.4;
             ...
             for(int ii=0; ii<nTracks; ii++)
               {
                ...
                if (phi_dist > 3.1415) phi_dist = 6.283-phi_dist;
                if(sqrt(pow(STau_2_ETA - track_Eta[ii], 2) + pow(phi_dist, 2)) < Track_dist_To_STau_2 )
                 {
                  Track_dist_To_STau_2 = sqrt(pow(STau_2_ETA - track_Eta[ii], 2) + pow(phi_dist, 2));
                  Track_Index_Matched_To_STau_2=ii;
                 }
               }

             // ******************************************************** CSC Clusters
             // Closeset CSC RecHitCluster to STau_1
             int CSC_Index_Matched_To_STau_1 = -100;
             double CSC_dist_To_STau_1 = 0.4;

             for(int ii=0; ii<nCscRechitClusters; ii++)
               {
                double phi_dist = fabs(STau_1_PHI - cscRechitClusterPhi[ii]);
                if (phi_dist > 3.1415) phi_dist = 6.283-phi_dist;
                if(sqrt(pow(STau_1_ETA - cscRechitClusterEta[ii], 2) + pow(phi_dist, 2)) < CSC_dist_To_STau_1 )
                 {
                  CSC_dist_To_STau_1 = sqrt(pow(STau_1_ETA - cscRechitClusterEta[ii], 2) + pow(phi_dist, 2));
                  CSC_Index_Matched_To_STau_1=ii;
                 }
               }

             // Closeset CSC RecHitCluster to STau_2
             int CSC_Index_Matched_To_STau_2 = -100;
             double CSC_dist_To_STau_2 = 0.4;

             for(int ii=0; ii<nCscRechitClusters; ii++)
               {
                double phi_dist = fabs(STau_2_PHI - cscRechitClusterPhi[ii]);
                if (phi_dist > 3.1415) phi_dist = 6.283-phi_dist;
                if(sqrt(pow(STau_2_ETA - cscRechitClusterEta[ii], 2) + pow(phi_dist, 2)) < CSC_dist_To_STau_2 )
                 {
                  CSC_dist_To_STau_2 = sqrt(pow(STau_2_ETA - cscRechitClusterEta[ii], 2) + pow(phi_dist, 2));
                  CSC_Index_Matched_To_STau_2=ii;
                 }
               }

             // ******************************************************** DT Clusters
             // Closeset DT RecHitCluster to STau_1
             int DT_Index_Matched_To_STau_1 = -100;
             double DT_dist_To_STau_1 = 0.4;

             for(int ii=0; ii<nDtRechitClusters; ii++)
               {
                double phi_dist = fabs(STau_1_PHI - dtRechitClusterPhi[ii]);
                if (phi_dist > 3.1415) phi_dist = 6.283-phi_dist;
                if(sqrt(pow(STau_1_ETA - dtRechitClusterEta[ii], 2) + pow(phi_dist, 2)) < DT_dist_To_STau_1 )
                 {
                  DT_dist_To_STau_1 = sqrt(pow(STau_1_ETA - dtRechitClusterEta[ii], 2) + pow(phi_dist, 2));
                  DT_Index_Matched_To_STau_1=ii;
                 }
               }

             // Closeset DT RecHitCluster to STau_2
             int DT_Index_Matched_To_STau_2 = -100;
             double DT_dist_To_STau_2 = 0.4;

             for(int ii=0; ii<nDtRechitClusters; ii++)
               {
                double phi_dist = fabs(STau_2_PHI - dtRechitClusterPhi[ii]);
                if (phi_dist > 3.1415) phi_dist = 6.283-phi_dist;
                if(sqrt(pow(STau_2_ETA - dtRechitClusterEta[ii], 2) + pow(phi_dist, 2)) < DT_dist_To_STau_2 )
                 {
                  DT_dist_To_STau_2 = sqrt(pow(STau_2_ETA - dtRechitClusterEta[ii], 2) + pow(phi_dist, 2));
                  DT_Index_Matched_To_STau_2=ii;
                 }
               }

             // ******************************************************** Fill NTuples

             if(STau1_InMuonSystem && Track_Index_Matched_To_STau_1 >=0)
               Matched_track -> Fill(track_Pt[Track_Index_Matched_To_STau_1], track_Eta[Track_Index_Matched_To_STau_1], track_Phi[Track_Index_Matched_To_STau_1], Track_dist_To_STau_1);
             else if(STau1_InMuonSystem && Track_Index_Matched_To_STau_1<0 && STau_1_PT>0)
               Matched_track -> Fill(0,5,5,5);

             if(STau2_InMuonSystem && Track_Index_Matched_To_STau_2 >=0)
               Matched_track -> Fill(track_Pt[Track_Index_Matched_To_STau_2], track_Eta[Track_Index_Matched_To_STau_2], track_Phi[Track_Index_Matched_To_STau_2], Track_dist_To_STau_2);
             else if(STau2_InMuonSystem && Track_Index_Matched_To_STau_2<0 && STau_2_PT>0)
               Matched_track -> Fill(0,5,5,5);


             // ******************************************************** Test Print
// , STau_1_decay_X, STau_1_decay_Y, STau_1_decay_Z,  STau_2_decay_X, STau_2_decay_Y, STau_2_decay_Z, 
//             if(STau1_InMuonSystem)
//              {
                cout<<endl<<endl;
                printf ("%10.3f %10.3f %10.3f %15d \n", STau_1_PT, STau_1_ETA, STau_1_PHI, STau1_InMuonSystem);
                printf ("%10.3f %10.3f %10.3f %15d \n", STau_2_PT, STau_2_ETA, STau_2_PHI, STau2_InMuonSystem);

                cout<<endl<<endl<<"----------------------- GenParticles --------------------"<<endl<<endl;
                for(int ii=0; ii<nGenParticle; ii++)
                  {
                   if(gParticlePt[ii]<5.0) continue;
                   //cout<<gParticleProdVertexX[ii]<<"   "<<gParticleDecayVertexX[ii]<<endl;
                   ///*, gParticleDecayVertexX[ii], gParticleDecayVertexY[ii], gParticleDecayVertexZ[ii]*//*, gParticleProdVertexX[ii], gParticleProdVertexY[ii], gParticleProdVertexZ[ii]*/
                   printf ("%10.3f %10.3f %10.3f %10d %10d %10d \n", gParticlePt[ii], gParticleEta[ii], gParticlePhi[ii], gParticleId[ii], gParticleStatus[ii], gParticleMotherId[ii]  );
                  }
                ...
                for(int ii=0; ii<nTracks; ii++)
                  {
                   ...
                   // printf ("%10.3f %10.3f %10.3f %10.3f %10.3f %10.3f %10.3f %10.3f %10.3f \n", track_Pt[ii], track_Eta[ii], track_Phi[ii], distt, track_nHits[ii], track_ndof[ii], track_chi2[ii], track_dzToPV[ii], track_dxyToBS[ii]);, trackVertexX[ii], trackVertexY[ii], trackVertexZ[ii]
